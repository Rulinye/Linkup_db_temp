...
- `docs/FUNCTION_LIST.md` — 함수 31개 + NFR 매핑표 (성용 검토용)
...
> **딩정 님은 로그인이랑 서버 없이 갈 것 같으니까 이에 맞춰서 DB 수정해 주실 수 있나요?**
>
> 정신적인 컨디션이 엄청 주관적이잖아요. 그래서 수면 시간까지 여기에 포함해서, 제일 좋을 때를 10으로 뒀을 때 오늘은 0~10에서 어느 정도냐, 이렇게 수치화를 시키는 건 어떨까요?
>
> input 시나리오 기구 여부 포함해서 수정
>
> — 정신적 컨디션 관련 데이터, 추가하기로 한 거 맞나요?
> — 네, 수면시간까지 정신적 컨디션에 포함해서 하나로요!
