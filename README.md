...
**프로젝트**: Team LinkUp — 장시간 좌식 직장인/학생을 위한 맞춤형 운동 추천 데스크톱 앱  
**문서 버전**: v1.0  
**최종 수정일**: 2026-04-03  
**작성자**: DB 담당
...
### 1.1 왜 클라우드 DB가 필요 없는가

본 프로젝트는 다음 세 가지 전제를 따릅니다:

- **Windows 데스크톱 앱** (웹 아님)
- **인터넷 연결 불필요** (모든 데이터 로컬)
- **SQLite 사용** (서버 없는 파일 기반 DB)

따라서 MySQL, PostgreSQL 등의 서버형 DB는 **불필요**합니다.  
개발 중 팀원 간 DB 동기화도 **Git으로 SQL 스크립트를 공유**하는 것만으로 충분합니다.
...
### 1.3 권장: GitHub 저장소 구조

팀 GitHub 저장소가 아직 없다면, **개인 저장소에 먼저 올리고** 팀 저장소가 준비되면 이전하는 것을 권장합니다.  
단순히 메신저로 파일을 보내면 버전 충돌 위험이 있으므로, Git 사용을 강력히 권장합니다.
...
*이 문서는 프로젝트 진행 중 수정 및 보강될 수 있습니다. 변경 사항은 Git 커밋 메시지에 기록해 주세요.*
